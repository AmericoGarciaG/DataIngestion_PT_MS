# PROJECT_ROOT/scripts/f02_manage_gcp_environment.py
import os
import subprocess
import sys
import time
...
# Para que funcione si se ejecuta con `python -m scripts.f02_manage_gcp_environment`
# desde la raíz del proyecto.
sys.path.append(str(Path(__file__).resolve().parent.parent))
from utils_general import run_command_in_dir, get_gcloud_project_config, get_input

# Nombres de las variables de entorno esperadas en .env
ENV_VAR_PROJECT_ID = "GOOGLE_CLOUD_PROJECT_ID" # Usado como candidato y como el efectivo si tiene éxito
...
def check_gcloud_auth_status() -> tuple[bool, str | None]:
    ...
    try:
        ...
        if auth_proc.returncode == 0 and auth_proc.stdout.strip():
            ...
            print(f"                       (STDOUT: '{auth_proc.stdout.strip()}', STDERR: '{auth_proc.stderr.strip()}')")
...
def verify_billing_account(billing_account_id: str, project_root_path: Path, active_gcloud_account: str | None) -> bool:
    ...
    gcloud_exe = shutil.which("gcloud.cmd") if os.name == 'nt' else shutil.which("gcloud")
    if not gcloud_exe: 
        print(f"  {SCRIPT_PREFIX}ERROR (verify_billing): gcloud no encontrado.")
        return False
    ...
    print(f"  {SCRIPT_PREFIX}Ejecutando: gcloud beta billing accounts describe {billing_account_id} ...") # Resumido para claridad
    try:
        ...
        if proc.returncode == 0 and stdout_cleaned.lower() == expected_output_name.lower():
            ...
            active_account_display = active_gcloud_account if active_gcloud_account else "la cuenta actualmente autenticada"
            print(f"  {SCRIPT_PREFIX}ERROR: Cuenta de facturación '{billing_account_id}' no se pudo verificar (esperado '{expected_output_name}').")
            print(f"         Salida del comando (stdout): '{stdout_cleaned}'")
            print(f"         Salida del comando (stderr): '{proc.stderr.strip()}'")
            print(f"         Código de retorno del comando: {proc.returncode}")
            print(f"         GUÍA: Verifica el ID '{ENV_VAR_BILLING_ACCOUNT}' en tu .env. Asegúrate de que {active_account_display}")
            print(f"               tenga al menos el rol 'Visualizador de cuentas de facturación' (Billing Account Viewer) sobre la cuenta de facturación '{billing_account_id}'.")
            return False
    except Exception as e:
...
def manage_project_lifecycle(project_root: Path) -> str | None:
    ...
    """
    auth_ok, active_user_account = check_gcloud_auth_status()
    if not auth_ok:
        return None
    
    env_path = project_root / "service" / ".env"
    if not env_path.exists(): print(f"{SCRIPT_PREFIX}ERROR: service/.env no encontrado."); return None
    ...
    if not project_id_from_env:
        print(f"{SCRIPT_PREFIX}ERROR: Variable '{ENV_VAR_PROJECT_ID}' debe estar definida en .env.")
        return None
    if not billing_account_id:
        print(f"{SCRIPT_PREFIX}ERROR: Variable '{ENV_VAR_BILLING_ACCOUNT}' debe estar definida en .env.")
        return None
    
    if not verify_billing_account(billing_account_id, project_root, active_user_account):
        return None 

    print(f"\n{SCRIPT_PREFIX}--- Trabajando con ID de Proyecto (de .env): '{project_id_from_env}' ---")
    gcloud_exe = shutil.which("gcloud.cmd") if os.name == 'nt' else shutil.which("gcloud")
    if not gcloud_exe: print(f"{SCRIPT_PREFIX} ERROR CRITICO: gcloud no encontrado"); return None
    ...
    try:
        ...
        if proc_describe.returncode == 0:
            ...
            if lifecycle_state == "ACTIVE":
                ...
                print(f"                       GUÍA: Actualiza '{ENV_VAR_PROJECT_ID}' en tu .env a un ID de proyecto nuevo/diferente y vuelve a ejecutar.")
                return None
        else:
            print(f"  {SCRIPT_PREFIX}[INFO] Proyecto '{project_id_from_env}' no encontrado. Se intentará crear.")
    except Exception as e:
    ...
    if not project_exists:
        ...
        if not run_command_in_dir(create_cmd, str(project_root)):
            print(f"  {SCRIPT_PREFIX}ERROR FATAL: No se pudo crear el proyecto '{project_id_from_env}'.")
            print(f"                       GUÍA: Verifica permisos, unicidad del ID, o si está en 'soft-delete'. Actualiza '{ENV_VAR_PROJECT_ID}' en .env y reintenta.")
            return None
        print(f"  {SCRIPT_PREFIX}[OK] Proyecto '{project_id_from_env}' creado. Esperando 30s...")
        time.sleep(30)
        project_is_active = True 

        print(f"\n{SCRIPT_PREFIX}Intentando vincular '{project_id_from_env}' a facturación '{billing_account_id}'...")
        link_billing_cmd = ["gcloud", "beta", "billing", "projects", "link", project_id_from_env, "--billing-account", billing_account_id]
        if not run_command_in_dir(link_billing_cmd, str(project_root)):
         ...
         if proc_billing_info.returncode == 0:
             try:
                ...
                if billing_data.get("billingEnabled"):
                    print(f"  {SCRIPT_PREFIX}[OK] Facturación ya está habilitada para el proyecto.")
                else: 
                ...
                print(f"  {SCRIPT_PREFIX}ADVERTENCIA: No se pudo parsear la info de facturación para '{project_id_from_env}'. STDOUT: {proc_billing_info.stdout.strip()}")
         else:
            print(f"  {SCRIPT_PREFIX}ADVERTENCIA: No se pudo obtener información de facturación para el proyecto '{project_id_from_env}'. STDERR: {proc_billing_info.stderr.strip()}")
    ...
    if project_is_active:
        ...
        if not run_command_in_dir(set_project_cmd, str(project_root), suppress_stdout_if_captured=True):
            print(f"  {SCRIPT_PREFIX}ERROR: No se pudo establecer '{project_id_from_env}' como proyecto por defecto en gcloud.")
            return None
        
        current_gcloud_project_config = get_gcloud_project_config()
        if current_gcloud_project_config == project_id_from_env:
            ...
            # --- NUEVO PASO: Configurar el Quota Project para ADC ---
            ...
            if not run_command_in_dir(set_quota_project_cmd, str(project_root), exit_on_error=False, suppress_stdout_if_captured=True):
                print(f"  {SCRIPT_PREFIX}ADVERTENCIA: No se pudo establecer '{project_id_from_env}' como Quota Project para ADC.")
                print(f"               Podrías necesitar ejecutar 'gcloud auth application-default set-quota-project {project_id_from_env}' manualmente.")
            else:
            ...
            print(f"GUÍA: La variable '{ENV_VAR_PROJECT_ID}' en tu archivo .env está establecida a '{project_id_from_env}'.")
            print(f"      Esta es la que usarán los scripts de Terraform.")
            return project_id_from_env
        else:
            print(f"  {SCRIPT_PREFIX}ERROR: Se intentó configurar '{project_id_from_env}', pero gcloud CLI reporta '{current_gcloud_project_config}'.")
            return None
...
def main():
    ...
    if not effective_project_id:
        ...
        print(f"{SCRIPT_PREFIX}Por favor, revisa los mensajes anteriores, ajusta tu archivo .env o la configuración/permisos de GCP y reintenta.")
        sys.exit(1)
    else:
        print(f"\n{SCRIPT_PREFIX}¡Proceso de gestión del proyecto GCP '{effective_project_id}' finalizado exitosamente!")
        print(f"{SCRIPT_PREFIX}Asegúrate de que la variable '{ENV_VAR_PROJECT_ID}' en tu .env esté establecida a: {effective_project_id}")
